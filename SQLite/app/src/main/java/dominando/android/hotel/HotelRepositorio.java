...
public class HotelRepositorio {
    ...
    private int atualizar(Hotel hotel){
        ...
        int linhasAfetadas = db.update(HotelSQLHelper.TABELA_HOTEL, cv,
                HotelSQLHelper.COLUNA_ID + " = ?", new String[]{String.valueOf(hotel.id)});
    ...
    public int excluir(Hotel hotel){
        SQLiteDatabase db = helper.getWritableDatabase();
        int linhasAfetadas = db.delete(HotelSQLHelper.TABELA_HOTEL,
                HotelSQLHelper.COLUNA_ID + " = ?", new String[]{String.valueOf(hotel.id)});
        db.close();
        return linhasAfetadas;
    }
