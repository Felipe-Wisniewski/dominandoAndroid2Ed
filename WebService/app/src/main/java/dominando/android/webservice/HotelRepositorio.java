...
public class HotelRepositorio {
    ...
    public CursorLoader buscar(Context ctx, String s){
        ...
        return new CursorLoader(ctx, HotelProvider.CONTENT_URI, null, where, whereArgs,
                HotelSQLHelper.COLUNA_NOME);
    }

    private static ContentValues getValues(Hotel hotel){
        ContentValues cv = new ContentValues();
        cv.put(HotelSQLHelper.COLUNA_NOME, hotel.name);
        cv.put(HotelSQLHelper.COLUNA_ENDERECO, hotel.address);
        cv.put(HotelSQLHelper.COLUNA_ESTRELAS, hotel.stars);
    ...
    public static Hotel hotelFromCursor(Cursor cursor){
        ...
        long idServidor = cursor.getLong(cursor.getColumnIndex(HotelSQLHelper.COLUNA_ID_SERVIDOR));

        Hotel hotel = new Hotel(id, nome, endereco, estrelas, idServidor, Hotel.Status.values()[status]);

        return hotel;
    }

    public long inserirLocal(Hotel hotel, ContentResolver cr){
        Uri uri = cr.insert(HotelProvider.CONTENT_URI, getValues(hotel));
