...
public class HotelHttp {
    ...
    private void enviarDadosPendentes() throws Exception{
        Cursor cursor = mContext.getContentResolver().query(HotelProvider.CONTENT_URI, null,
                HotelSQLHelper.COLUNA_STATUS + " != " + Hotel.Status.OK.ordinal(),
                null, HotelSQLHelper.COLUNA_ID_SERVIDOR + " DESC");
    ...
    private HttpURLConnection abrirConexao(String url, String metodoHttp, boolean doOutput)
            throws Exception{
    ...
    private byte[] hotelToJsonBytes(Hotel hotel){
        try {
            ...
            jsonHotel.put("nome", hotel.name);
            jsonHotel.put("endereco", hotel.address);
            jsonHotel.put("estrelas", hotel.stars);
